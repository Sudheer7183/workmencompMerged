...
class TenantCarrierItem(BaseModel):
    ...
    carrier_name: str
    carrier_slug: str
    carrier_code: str   # alias for carrier_slug — matches frontend TenantCarrier interface
    is_active: bool
...
async def get_tenant_carriers(
    ...
    result = await db.execute(
        text(
            "SELECT tc.carrier_id, c.carrier_name, c.slug, tc.is_active "
    ...
    return [
        {
            "carrier_id": r[0],
            "carrier_name": r[1],
            "carrier_code": r[2],   # frontend TenantCarrier interface uses carrier_code
            "carrier_slug": r[2],   # keep carrier_slug for backward compat
            "is_active": r[3],
        }
        for r in rows
    ]
...
# ---------------------------------------------------------------------------
# Phase 7A — TENANT_ADMIN Carrier Management endpoints
# ---------------------------------------------------------------------------

class TenantCarrierAddRequest(BaseModel):
    """Request body for POST /api/v1/admin/carriers."""
    carrier_id: int = Field(..., gt=0)


class TenantCarrierCreateRequest(BaseModel):
    """
    Request body for POST /api/v1/tenant/carriers/create.
    Creates a new carrier in public.carriers and immediately assigns it
    to this tenant — single atomic operation for TENANT_ADMIN self-service.
    """
    carrier_name: str = Field(..., min_length=1, max_length=200)
    slug: str = Field(
        ...,
        pattern=r"^[a-z0-9][a-z0-9\-]{0,61}[a-z0-9]?$",
        description="URL-safe identifier, e.g. 'acme-insurance'",
    )


class AvailableCarrierItem(BaseModel):
    """A platform carrier not yet assigned to this tenant."""
    carrier_id: int
    carrier_name: str
    slug: str


@router.get("/carriers/available", response_model=list[AvailableCarrierItem])
async def list_available_carriers(
    ...
    """
    Returns platform carriers not yet actively assigned to this tenant.
    Used by CarrierManagementPanel to populate the 'Add a Carrier' selector.
    ...
    result = await db.execute(
        text(
            "SELECT c.carrier_id, c.carrier_name, c.slug "
            "FROM public.carriers c "
            "WHERE c.carrier_id NOT IN ("
            "  SELECT tc.carrier_id FROM tenant_carriers tc WHERE tc.is_active = TRUE"
            ") "
            "ORDER BY c.carrier_name"
        )
    )
    rows = result.fetchall()
    return [
        {"carrier_id": r[0], "carrier_name": r[1], "slug": r[2]}
        for r in rows
    ]
...
async def add_carrier_to_tenant(
    request: Request,
    body: TenantCarrierAddRequest,
    token: TokenPayload = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> dict[str, Any]:
    """
    Associates a platform carrier with this tenant and seeds all required
    carrier-level configuration (calc config, 22 calc rules, theme config).

    Idempotent: if the carrier was previously removed (is_active=FALSE),
    it is re-activated. Existing calc rules and theme config are never
    overwritten — ON CONFLICT DO NOTHING preserves admin edits.
    ...
    # Verify the carrier_id exists on the platform
    carrier_check = await db.execute(
        text("SELECT carrier_name, slug FROM public.carriers WHERE carrier_id = :cid"),
        {"cid": body.carrier_id},
    )
    carrier_row = carrier_check.fetchone()
    if carrier_row is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Carrier with id {body.carrier_id} does not exist on the platform.",
        )

    from app.services.tenant_provisioning_service import TenantProvisioningService
    from app.services.keycloak_admin_service import get_keycloak_admin_service

    provisioning_svc = TenantProvisioningService(
        keycloak_service=get_keycloak_admin_service()
    )
    await provisioning_svc.add_carrier_to_tenant(
        schema_name=schema,
    ...
    return {
        ...
        "slug": carrier_row[1],
        "is_active": True,
        "message": "Carrier added successfully. Calculation rules and configuration seeded.",
    }
...
@router.post(
    ...
    summary="Create a new platform carrier and assign it to this tenant (TENANT_ADMIN)",
)
async def create_and_assign_carrier(
    body: TenantCarrierCreateRequest,
    request: Request,
    token: TokenPayload = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    ...
    Two-step atomic operation:
      1. INSERT the new carrier into public.carriers (using the public DB session).
      2. Assign it to this tenant via add_carrier_to_tenant() (using the tenant DB session).

    Both steps succeed or neither is committed.
    The slug must be globally unique across all platform carriers.
    ...
    existing = await public_db.execute(
        text("SELECT carrier_id FROM public.carriers WHERE slug = :slug"),
        {"slug": body.slug},
    )
    if existing.fetchone() is not None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Carrier slug '{body.slug}' is already taken. Choose a different identifier.",
        )

    # ── Step 2: Create carrier in public schema ─────────────────────────────
    result = await public_db.execute(
        text(
            "INSERT INTO public.carriers (carrier_name, slug, ai_narrative_enabled) "
            "VALUES (:name, :slug, FALSE) "
            "RETURNING carrier_id, carrier_name, slug"
        ),
        {"name": body.carrier_name, "slug": body.slug},
    )
    await public_db.commit()
    row = result.fetchone()
    new_carrier_id: int = row[0]

    # ── Step 3: Assign to this tenant ──────────────────────────────────────
    from app.services.tenant_provisioning_service import TenantProvisioningService
    from app.services.keycloak_admin_service import get_keycloak_admin_service
    provisioning_svc = TenantProvisioningService(get_keycloak_admin_service())
    await provisioning_svc.add_carrier_to_tenant(
    ...
    return {
        ...
        "slug": row[2],
        "assigned": True,
        "message": f"Carrier '{body.carrier_name}' created and assigned to your organisation.",
    }

@router.delete("/carriers/{carrier_id}", status_code=status.HTTP_204_NO_CONTENT, response_model=None)
async def remove_carrier_from_tenant(
    ...
    """
    Soft-removes a carrier association from this tenant.
    Sets tenant_carriers.is_active = FALSE. Data is preserved.
    The carrier can be re-added later via POST /api/v1/admin/carriers.
    ...
    result = await db.execute(
        text(
            "SELECT is_active FROM tenant_carriers WHERE carrier_id = :cid"
        ),
        {"cid": carrier_id},
    )
    row = result.fetchone()
    if row is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Carrier {carrier_id} is not assigned to this tenant.",
        )
    if not row[0]:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Carrier {carrier_id} is already inactive for this tenant.",
        )
