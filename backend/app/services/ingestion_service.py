        ...
        await db.execute(
            text("""
                UPDATE policies SET
                    ...
                    premium_written   = COALESCE(:prem,  premium_written)
                WHERE policy_id = :pid
            """),
            {
                "sc":    state_code,
                "freq":  safe_freq,
                "owner": safe_owner,
                "prem":  float(total_premium) if total_premium else None,
                "pid":   policy_id,
            },
        )
        await db.commit()
...
# ===========================================================================
# Phase 4 extensions — appended below the Phase 3 IngestionService class.
# These methods are monkey-patched onto IngestionService after definition.
# ===========================================================================
# NOTE: The Phase 3 IngestionService.run() only accepts "xlsx" | "xml".
# Phase 4 extends it to accept "csv" as well.
# We override run() and run_post_approval() by subclassing.
...
class IngestionServiceV4(IngestionService):
    """
    Phase 4 extension of IngestionService.

    Changes from Phase 3:
      1. run() accepts file_type='csv' in addition to 'xlsx' and 'xml'.
      2. run_post_approval() wraps per-row upsert logic with try/except that
         writes to ingestion_errors + ingestion_skipped_rows when skip_on_error=TRUE.
         When skip_on_error=FALSE, any error aborts the entire run.
      3. _parse_and_upsert_csv() added for CSV ingestion.
      4. Per-row error tracking helper _record_skipped_row() added.
    """
    ...
    async def run(
        ...
        """
        Phase 4: Accepts xlsx | xml | csv.
        Returns (run_id, session_id) — same contract as Phase 3.
        """
        from sqlalchemy import text as _text

        if file_type not in ("xlsx", "xml", "csv"):
            raise ValueError(f"Unsupported file type: '{file_type}'. Accepted: xlsx, xml, csv")
        ...
        # Store skip_on_error flag on the run record.
        if skip_on_error:
            await db.execute(
                _text("UPDATE ingestion_runs SET skip_on_error = TRUE WHERE run_id = :rid"),
                {"rid": run_id},
            )
            await db.commit()
        ...
        try:
            ...
            try:
                await db.execute(
                    _text("UPDATE ingestion_runs SET status='failed', error_detail=:err WHERE run_id=:rid"),
                    {"err": str(exc)[:2000], "rid": run_id},
                )
                await db.commit()
            except Exception:
    ...
    async def run_post_approval(
        ...
        """
        Phase 4 override: Adds per-row exception tracking.

        When skip_on_error=TRUE: skipped rows are logged to ingestion_skipped_rows
        and ingestion_errors. Run completes as 'partial'.
        When skip_on_error=FALSE: any row error fails the entire run.
        """
        from sqlalchemy import text as _text
        ...
        # Read skip_on_error flag from the run record.
        run_row = await db.execute(
            _text("SELECT skip_on_error FROM ingestion_runs WHERE run_id = :rid"),
            {"rid": run_id},
        )
        run_record = run_row.fetchone()
        skip_on_error: bool = bool(run_record[0]) if run_record else False
        ...
        try:
            ...
            elif file_bytes and self._is_csv(file_bytes):
                rows_ingested = await self._parse_and_upsert_csv(
                    carrier_id=carrier_id, run_id=run_id,
                    file_bytes=file_bytes, proposals=proposals,
                    db=db, schema_name=schema_name, skip_on_error=skip_on_error,
                )
            elif file_bytes:
                rows_ingested = await self._parse_and_upsert_xlsx(
                    carrier_id=carrier_id, run_id=run_id,
            ...
            # Determine final status: 'partial' if any rows were skipped.
            skipped_count_row = await db.execute(
                _text("SELECT COUNT(*) FROM ingestion_skipped_rows WHERE run_id = :rid"),
                {"rid": run_id},
            )
            skipped_count: int = skipped_count_row.scalar() or 0

            if skipped_count > 0:
                await db.execute(
                    _text("""
                        UPDATE ingestion_runs
                        SET status='partial', rows_ingested=:rows, rows_skipped=:skipped, completed_at=now()
                        WHERE run_id=:rid
                    """),
                    {"rows": rows_ingested, "skipped": skipped_count, "rid": run_id},
                )
                await db.commit()
            else:
                await self._complete_run(run_id, rows_ingested, db, schema_name=schema_name)
    ...
    @staticmethod
    def _is_csv(file_bytes: bytes) -> bool:
        """
        Returns True when the file appears to be CSV (not XLSX, not XML).
        Heuristic: XLSX files start with PK (zip magic); XML starts with < .
        Anything else that decodes as UTF-8 text is treated as CSV.
        """
        if file_bytes[:2] == b"PK":
            return False
        if file_bytes[:1] in (b"<",):
            return False
        try:
            # Strip UTF-8 BOM if present (Excel-saved CSV files).
            sample = file_bytes[:512].lstrip(b"\xef\xbb\xbf").decode("utf-8", errors="strict")
            return bool(sample.strip())
        except UnicodeDecodeError:
            return False

    async def _parse_and_upsert_csv(
        self,
        carrier_id: int,
        run_id: int,
        file_bytes: bytes,
        ...
        """
        CSV ingestion — uses csv.reader per V9 S16.5 spec.

        CSV handling per prompt spec:
          - Decodes with 'utf-8-sig' which handles UTF-8 BOM natively (Excel-saved CSVs).
          - Uses csv.reader (not DictReader) — rows are lists, headers from first data row.
          - delimiter: from ingestion_sources; defaults to ','.
          - Uses the same canonical-column routing as _process_sheet().
          - Per-row error tracking when skip_on_error=TRUE.
        """
        from sqlalchemy import text as _text

        # Decode with utf-8-sig — Python's built-in BOM handling per spec
        text_content = file_bytes.decode("utf-8-sig", errors="replace")

        # Build mapping lookup from approved proposals
        mapping_lookup: dict[str, str] = {
            p["source_field"]: p["proposed_target"]
            for p in proposals
            if p["proposed_target"]
        ...
        # Parse CSV using csv.reader as specified
        import io as _io_module
        reader = _csv.reader(_io_module.StringIO(text_content), delimiter=",")
        all_rows = list(reader)
        if not all_rows:
            logger.warning("ingestion.csv.empty", run_id=run_id)
            return 0

        # First row is headers
        headers_ordered = [str(h).strip() for h in all_rows[0]]

        # Build canonical_col_map: {canonical_name: 1-based col index}
        col_index_map: dict[str, int] = {}
        for col_idx, header in enumerate(headers_ordered, start=1):
            if not header:
                continue
            if header in mapping_lookup:
                col_index_map[mapping_lookup[header]] = col_idx
            else:
                norm = "".join(c for c in header.lower() if c.isalnum() or c == " ").strip()
                for src_field, canonical in mapping_lookup.items():
                    src_norm = "".join(c for c in src_field.lower() if c.isalnum() or c == " ").strip()
                    if norm == src_norm:
                        col_index_map[canonical] = col_idx
                        break

        if not col_index_map:
            logger.warning("ingestion.csv.no_mapped_columns", run_id=run_id)
            return 0

        logger.info("ingestion.csv.routing", run_id=run_id, canonical_cols=list(col_index_map.keys()))

        # Data rows: skip header row, convert to tuples for reuse of row upsert methods
        rows: list[tuple] = []
        row_errors: list[dict] = []
        for row_num, raw_row in enumerate(all_rows[1:], start=2):
            try:
                # Pad/trim row to match header count
                padded = raw_row + [""] * max(0, len(headers_ordered) - len(raw_row))
                rows.append(tuple(padded[:len(headers_ordered)]))
            except Exception as exc:
        ...
        # Use the same routing logic as _process_sheet but operating on the collected tuples.
        count = await self._process_csv_rows(
            rows=rows,
        ...
        # Record any row-parse errors.
        for err in row_errors:
            await self._record_error(
                run_id=run_id,
                row_number=err["row"],
                error_type=IngestionErrorCode.INTERNAL_ERROR,
    ...
    async def _process_csv_rows(
        ...
        """Routes CSV rows using canonical col map — same logic as _process_sheet."""
        from sqlalchemy import text as _text

        total = 0

        if "wages" in col_index_map and "class_code" in col_index_map:
            # Mock a worksheet-compatible interface using a simple namespace.
            total += await self._upsert_payroll_class_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        elif "est_payroll" in col_index_map or "actual_payroll_reported" in col_index_map:
            total += await self._upsert_payroll_policy_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        elif "est_premium_end" in col_index_map or "actual_premium" in col_index_map:
            total += await self._upsert_premium_variance_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        elif "reason_code" in col_index_map or "report_date" in col_index_map:
            total += await self._upsert_zero_payroll_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        elif "expected_period_start" in col_index_map or "days_overdue" in col_index_map:
            total += await self._upsert_missing_payroll_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        elif "policy_number" in col_index_map:
            total += await self._upsert_policies_only_from_rows(
                rows, col_index_map, carrier_id, run_id, db, skip_on_error, schema_name
            )
        else:
            logger.warning("ingestion.csv.unroutable", canonical_cols=list(col_index_map.keys()))

        return total

    # =========================================================================
    # Row-level error recording helpers
    # =========================================================================

    async def _record_error(
        self,
        run_id: int,
        error_type: IngestionErrorCode,
        error_message: str,
        db: AsyncSession,
        ...
        """Inserts one row into ingestion_errors."""
        from sqlalchemy import text as _text

        await db.execute(
            _text("""
                INSERT INTO ingestion_errors
                  (run_id, row_number, field_name, error_type, error_message, raw_value, created_at)
                VALUES (:rid, :rnum, :field, :etype, :emsg, :rval, now())
            """),
            {
                "rid": run_id,
                "rnum": row_number,
                "field": field_name,
                "etype": error_type.value,
                "emsg": error_message[:2000],
                "rval": str(raw_value)[:500] if raw_value is not None else None,
            },
        )

    async def _record_skipped_row(
        self,
        run_id: int,
        row_number: int,
        raw_data: dict,
        ...
        """Inserts one row into ingestion_skipped_rows."""
        import json
        from sqlalchemy import text as _text
        ...
        await db.execute(
            _text("""
                ...
                VALUES (:rid, :rnum, :rdata::jsonb, :reason, :codes, 'PENDING')
            """),
            {
                "rid": run_id,
                "rnum": row_number,
                "rdata": json.dumps(raw_data),
                "reason": skip_reason[:2000],
                "codes": error_codes,
            },
        )

    # =========================================================================
    # Row-list upsert variants (for CSV rows — same logic without ws.iter_rows)
    # =========================================================================

    async def _upsert_policies_only_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        count = 0
        for row_num, row in enumerate(rows, start=2):
            ...
            try:
                ...
                if skip_on_error:
                    raw = {str(i): str(v) for i, v in enumerate(row)}
                    await self._record_error(run_id, IngestionErrorCode.POLICY_UPSERT_FAILED, str(exc), db, row_num)
                    await self._record_skipped_row(run_id, row_num, raw, str(exc), [IngestionErrorCode.POLICY_UPSERT_FAILED.value], db)
                else:
                    raise
        await db.commit()
        return count

    async def _upsert_premium_variance_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        from sqlalchemy import text as _text
        ...
        for row_num, row in enumerate(rows, start=2):
            ...
            try:
                pid = await self._ensure_policy(row, cmap, carrier_id, db)
                if pid is None:
                    raise IngestionRowError(IngestionErrorCode.MISSING_POLICY_NUMBER, "No policy_number", {})
                est = _safe_decimal(self._cv(row, cmap, "est_premium_end"))
                actual = _safe_decimal(self._cv(row, cmap, "actual_premium", "premium_written"))
                if est is None or actual is None:
                    raise IngestionRowError(IngestionErrorCode.MISSING_REQUIRED_FIELD, "est_premium_end or actual_premium missing", {})
                as_of = _safe_date(self._cv(row, cmap, "as_of_date", "effective_date")) or date.today()
                await db.execute(_text("""
                    ...
                    ON CONFLICT (policy_id, ingestion_run_id) DO UPDATE SET
                      est_premium_end = EXCLUDED.est_premium_end,
                      actual_premium  = EXCLUDED.actual_premium
                """), {"pid": pid, "cid": carrier_id, "rid": run_id, "dt": as_of, "est": float(est), "actual": float(actual)})
                count += 1
            except Exception as exc:
                if skip_on_error:
                    raw = {str(i): str(v) for i, v in enumerate(row)}
                    code = exc.code if isinstance(exc, IngestionRowError) else IngestionErrorCode.INTERNAL_ERROR
                    await self._record_error(run_id, code, str(exc), db, row_num)
                    await self._record_skipped_row(run_id, row_num, raw, str(exc), [code.value], db)
                else:
                    raise
        await db.commit()
        return count

    async def _upsert_payroll_policy_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        from sqlalchemy import text as _text
            ...
            try:
                ...
                await db.execute(_text("""
                    ...
                    ON CONFLICT (policy_id, ingestion_run_id) DO UPDATE SET
                      ...
                      actual_payroll_reported = EXCLUDED.actual_payroll_reported,
                      actual_payroll_classified = EXCLUDED.actual_payroll_classified
                """), {"pid": pid, "cid": carrier_id, "rid": run_id, "dt": as_of, "est": float(est), "rep": float(reported), "cls": float(classfd)})
                count += 1
            except Exception as exc:
                if skip_on_error:
                    raw = {str(i): str(v) for i, v in enumerate(row)}
    ...
    async def _upsert_payroll_class_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        from collections import defaultdict
        ...
        agg: dict[tuple, dict] = defaultdict(lambda: {"wages": Decimal("0"), "exposure": Decimal("0"), "as_of": None})
        policy_cache: dict[str, Optional[int]] = {}
        ...
        for (pid, state, cls_str), totals in agg.items():
            ...
            await db.execute(_text("""
                INSERT INTO payroll_variance_class
                  (policy_id, carrier_id, ingestion_run_id, class_code_id,
                   state_code, as_of_date, est_payroll, actual_reported, actual_classified)
                VALUES (:pid, :cid, :rid, :ccid, :state, :dt, 0, :wages, :exposure)
                ON CONFLICT (policy_id, state_code, class_code_id, ingestion_run_id) DO UPDATE SET
                  actual_reported = payroll_variance_class.actual_reported + EXCLUDED.actual_reported,
                  actual_classified = payroll_variance_class.actual_classified + EXCLUDED.actual_classified
            """), {"pid": pid, "cid": carrier_id, "rid": run_id, "ccid": class_code_id,
                  "state": state, "dt": as_of, "wages": float(totals["wages"]), "exposure": float(totals["exposure"])})
            count += 1
        await db.commit()
        return count

    async def _upsert_zero_payroll_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        from sqlalchemy import text as _text
        ...
        for row_num, row in enumerate(rows, start=2):
            ...
            try:
                ...
                await db.execute(_text("""
                    INSERT INTO zero_payroll
                      (policy_id, carrier_id, ingestion_run_id, policyholder_name, policy_number, state_code, report_date, payroll_frequency)
                    VALUES (:pid, :cid, :rid, :name, :pnum, :state, :dt, :freq)
                """), {"pid": pid, "cid": carrier_id, "rid": run_id, "name": insured, "pnum": pol_num, "state": state, "dt": rep_date, "freq": freq})
                count += 1
            except Exception as exc:
                if skip_on_error:
                    raw = {str(i): str(v) for i, v in enumerate(row)}
                    await self._record_error(run_id, IngestionErrorCode.INTERNAL_ERROR, str(exc), db, row_num)
                    await self._record_skipped_row(run_id, row_num, raw, str(exc), [IngestionErrorCode.INTERNAL_ERROR.value], db)
                else:
                    raise
        await db.commit()
        return count

    async def _upsert_missing_payroll_from_rows(
        self, rows: list[tuple], cmap: dict[str, int], carrier_id: int, run_id: int,
        db: AsyncSession, skip_on_error: bool, schema_name: str,
    ) -> int:
        from sqlalchemy import text as _text
        ...
        for row_num, row in enumerate(rows, start=2):
            ...
            try:
                ...
                await db.execute(_text("""
                    INSERT INTO missing_payroll
                      (policy_id, carrier_id, ingestion_run_id, policyholder_name, policy_number, state_code,
                       period_start, period_end, days_since_last_run, payroll_frequency)
                    VALUES (:pid, :cid, :rid, :name, :pnum, :state, :ps, :pe, :days, :freq)
                """), {"pid": pid, "cid": carrier_id, "rid": run_id, "name": insured, "pnum": pol_num, "state": state,
                      "ps": p_start, "pe": p_end, "days": int(days) if days is not None else None, "freq": freq})
                count += 1
            except Exception as exc:
                if skip_on_error:
                    raw = {str(i): str(v) for i, v in enumerate(row)}
