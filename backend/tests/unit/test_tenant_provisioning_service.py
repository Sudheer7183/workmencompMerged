...
async def test_provision_seeds_22_rules_per_carrier(mock_subprocess_alembic):
    """After provisioning, exactly 22 calculation rules must be inserted per carrier."""
    ...
    with (
        ...
        await svc._seed_tenant_schema("tenant_alpha", [1], db)
...
async def test_provision_seeds_carrier_theme_config(mock_subprocess_alembic):
    """carrier_theme_config must be seeded with theme_source='SYSTEM', default_theme_id=1."""
    svc = _build_service()
    db = _make_mock_db()
