...
def test_policy_list_returns_200(client, app):
    mock_db = _make_mock_db(
        scalar_one=1,
        fetchall=[
            # Columns: policy_id, policy_number, insured_name, state_code, effective_date,
            #          policy_status, est_premium, actual_premium (r[7] skipped in mapping),
            #          variance_amount, variance_pct, risk_level, audit_status
            (1, "WC-001", "Acme Corp", "CA", date(2024, 1, 1),
             "Active", Decimal("100000"), Decimal("105000"), Decimal("5000"), Decimal("0.05"), "Medium", "Pending"),
        ],
    )
...
def test_variance_pct_null_when_engine_off(client, app):
    mock_db = _make_mock_db(
        scalar_one=1,
        fetchall=[
            # Columns match the SELECT: ..., est_premium, actual_premium (r[7]),
            #          variance_amount, variance_pct, risk_level, audit_status
            (1, "WC-001", "Acme", "CA", date(2024, 1, 1),
             "Active", Decimal("100000"), Decimal("100000"), None, None, None, "Pending"),
        ],
    )
    _apply_overrides(app, mock_db, _reviewer())
    with (
