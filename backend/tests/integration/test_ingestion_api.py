...
def _make_mock_db(fetchall=None, scalar_one=None, fetchone=None) -> AsyncMock:
    cursor = MagicMock()
    cursor.fetchall.return_value = fetchall if fetchall is not None else []
    cursor.scalar_one.return_value = scalar_one
    # fetchone must default to None so that the carrier mode-lock check
    # (which calls fetchone() on the ingestion_runs query) does not
    # produce a MagicMock truthy value and erroneously raise 409.
...
def test_upload_rejects_unsupported_file_type(client, app):
    """Truly unsupported file type (PDF) must be rejected with 422.
    Note: Phase 4 expanded accepted types to xlsx, xml, and csv.
    Only unknown/unrecognised MIME types now return 422."""
    mock_db = _make_mock_db()
