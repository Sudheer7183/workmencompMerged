    ...
    return (
      <div className="calc-engine-toggle calc-engine-toggle--error">
        <span className="calc-engine-toggle__error-text">
          {labels.calc_engine_load_error ?? "Failed to load engine config"}
        </span>
      </div>
    );
  }
    ...
    <div className="calc-engine-toggle">
      <div className="calc-engine-toggle__header">
        <div className="calc-engine-toggle__title-group">
          <h3 className="calc-engine-toggle__title">
            {labels.calc_engine_title ?? "Calculation Engine"}
          </h3>
          <p className="calc-engine-toggle__description">
            {labels.calc_engine_description ??
        ...
        <div className="calc-engine-toggle__control">
          ...
          <button
            ...
            aria-label={
              engineOn
                ? (labels.calc_engine_on ?? "Engine enabled — click to disable")
                : (labels.calc_engine_off ?? "Engine disabled — click to enable")
            }
            className={[
          ...
          >
            {isToggling
              ? (labels.calc_engine_saving ?? "Saving…")
              : engineOn
              ? (labels.calc_engine_status_on ?? "ON")
              : (labels.calc_engine_status_off ?? "OFF")}
          </span>
        </div>
      </div>
      ...
      {!engineOn && (
        <div className="calc-engine-toggle__warning" role="alert">
          ...
          <span>
            {labels.calc_engine_off_warning ??
      ...
      {effectiveReadonly && (
        <p className="calc-engine-toggle__readonly">
          {labels.calc_engine_readonly_note ??
