...
export function Step1TenantDetails({ initial, onNext }: Props): React.JSX.Element {
  ...
  const slugErrorMsg =
    slugStatus === "taken"
      ? labels.platform_subdomain_taken
      : slugStatus === "reserved"
      ? labels.platform_subdomain_reserved
      : slugStatus === "invalid"
  ...
  return (
    <form className="wizard-step" onSubmit={handleSubmit}>
      <h2 className="wizard-step__title">{labels.platform_wizard_step1}</h2>

      <div className="form-field">
        <label className="form-field__label" htmlFor="tenant-name">
          {labels.platform_field_tenant_name}
        </label>
        <input
          id="tenant-name"
      ...
      <div className="form-field">
        <label className="form-field__label" htmlFor="tenant-slug">
          {labels.platform_field_subdomain}
        </label>
        <input
          id="tenant-slug"
        ...
        />
        {slugStatus === "checking" && (
          <p className="form-field__hint">{labels.loading}</p>
        )}
        ...
        <p className="form-field__note">{labels.platform_subdomain_immutable_note}</p>
      </div>

      <div className="form-field">
        <label className="form-field__label" htmlFor="tenant-type">
          {labels.platform_field_tenant_type}
        </label>
        <select
          id="tenant-type"
