...
describe("TenantCreationWizard", () => {
  it("renders 4-step progress indicator", () => {
    ...
    // DEFAULT_LABELS renders "Carrier Assignment (Optional)" - use regex to match
    expect(screen.getAllByText(/carrier assignment/i).length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText(/review.*activate/i).length).toBeGreaterThanOrEqual(1);
  });
  ...
  it("step 4: clicking Activate shows provisioning progress overlay", async () => {
    ...
    await waitFor(() => {
      // While provisioning runs, the Activate button changes to "Activating…"
